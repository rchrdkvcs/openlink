...
<template>
    ...
    <AuthenticatedLayout>
        <div class="mx-auto w-full max-w-2xl px-4 py-8 sm:px-6">
            <Link :href="route('domains.index')" class="mb-6 inline-flex items-center gap-1.5 text-sm text-muted hover:text-foreground">
            ...
            <div class="card-sheen rounded-xl border bg-surface p-6">
                ...
                <template v-if="step === 1">
                    ...
                    <p class="mt-1 text-sm text-muted">
                        ...
                        <code class="rounded bg-elevated px-1 py-0.5 font-mono text-xs">go.yourcompany.com</code> —
                        you must own the domain to complete the next step.
                    </p>
                ...
                <template v-else-if="step === 2 && domain">
                    ...
                    <p class="mt-1 text-sm text-muted">
                        Sign in to the website where you bought your domain (GoDaddy, OVH, Cloudflare, Namecheap…),
                        find the <strong>DNS settings</strong>, and add both records below. You only need to do this once.
                    </p>
                    ...
                    <div class="mt-5 space-y-4">
                        ...
                        >
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <Badge :variant="record.done ? 'success' : 'warning'" dot>{{ record.done ? 'found' : 'waiting' }}</Badge>
                            </div>
                            ...
                            <dl class="mt-3 grid gap-2 text-xs">
                                <div class="grid grid-cols-[64px_1fr_auto] items-center gap-2">
                                    ...
                                    <dd><code class="rounded bg-elevated px-1.5 py-0.5 font-mono">{{ record.type }}</code></dd>
                                    <span />
                                </div>
                                <div class="grid grid-cols-[64px_1fr_auto] items-center gap-2">
                            ...
                            <p v-if="record.error && !record.done" class="mt-2 text-xs text-warning">{{ record.error }}</p>
                        </div>
                    </div>

                    <div class="mt-5 flex items-center justify-between gap-3">
                        <p class="text-xs text-faint">
                            We check automatically every 15 seconds. DNS changes usually apply within minutes,
                ...
                <template v-else-if="domain">
                    <div class="flex flex-col items-center py-4 text-center">
                        ...
                        <p class="mt-1 max-w-sm text-sm text-muted">
                            Your domain is verified and pointing to this server. You can now create short links on it.
                            HTTPS may take a few minutes to become available on the first visit.
                        </p>
                        <div class="mt-6 flex gap-3">
                            <Button variant="secondary" @click="router.visit(route('domains.index'))">View domains</Button>
                            <Button @click="router.visit(route('links.index'))">
                                <Link2 class="h-4 w-4" /> Create a link
                            </Button>
                        </div>
                    </div>
                </template>
