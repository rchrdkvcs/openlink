...
<template>
    <section>
        <header>
            ...
            <p class="mt-1 text-sm text-muted">Protect your account with a time-based one-time password app.</p>
        </header>

        <div class="mt-6 space-y-5">
            <div v-if="twoFactor.enabled" class="rounded-md border border-success/25 bg-success/10 px-4 py-3 text-sm text-success">
                Two-factor authentication is enabled.
            </div>

            <div v-else-if="twoFactor.pendingSecret" class="space-y-4">
                <div class="rounded-md border border-warning/25 bg-warning/10 p-4">
                    <p class="text-sm font-medium text-warning">Add this secret to your authenticator app, then enter a code to confirm.</p>
                    <code class="mt-3 block break-all rounded-md border bg-background px-3 py-2 font-mono text-sm text-foreground">{{ twoFactor.pendingSecret }}</code>
                    <p v-if="twoFactor.otpauthUrl" class="mt-2 break-all font-mono text-xs text-faint">{{ twoFactor.otpauthUrl }}</p>
                </div>

                <form class="flex max-w-sm items-end gap-3" @submit.prevent="confirm">
                    <div class="flex-1">
                        <InputLabel for="two_factor_code" value="Authentication code" />
                        <TextInput id="two_factor_code" v-model="confirmForm.code" class="mt-1.5 block w-full" inputmode="numeric" />
            ...
            <form v-if="twoFactor.enabled" class="flex max-w-sm items-end gap-3" @submit.prevent="disable">
                <div class="flex-1">
                    ...
                    <TextInput id="disable_two_factor_password" v-model="disableForm.password" type="password" class="mt-1.5 block w-full" />
