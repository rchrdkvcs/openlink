...
import { SERIES_COLORS, formatBucket, formatCompact, formatNumber, type ReportRange, type TimePoint } from '@/lib/analytics';
...
const tooltip = computed(() => {
    ...
    return { point, anchor, flip, left: flip ? undefined : `${anchor + 12}px`, right: flip ? `${width.value - anchor + 12}px` : undefined };
});
</script>
...
<template>
    <div ref="wrapper" class="relative">
        ...
        >
            ...
            <path :d="linePath('visits')" fill="none" :stroke="SERIES_COLORS.visits" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" />
            ...
            <g v-if="active !== null && points[active]">
                <line :x1="x(active)" :x2="x(active)" :y1="pad.top" :y2="pad.top + plotH" class="stroke-border-strong" stroke-width="1" />
                <circle :cx="x(active)" :cy="y(points[active].visits)" r="4" :fill="SERIES_COLORS.visits" class="stroke-surface" stroke-width="2" />
                <circle
                    v-if="hasScans"
                    :cx="x(active)"
