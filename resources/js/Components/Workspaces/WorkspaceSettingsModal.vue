    ...
    <Modal :show="show" max-width="lg" @close="emit('close')">
        <div class="p-6">
            <div class="flex items-start gap-3">
                <WorkspaceIconPicker v-if="manage" v-model:icon="form.icon" :name="form.name || manage.name" :color="form.color" />
                <WorkspaceAvatar v-else size="lg" />
                <div class="min-w-0">
                    <h2 class="truncate text-base font-semibold text-foreground">Workspace settings</h2>
                    <p class="mt-0.5 text-sm text-muted">Name, appearance, and defaults for {{ manage?.name ?? 'this workspace' }}. Click the icon to change it.</p>
                </div>
            </div>
            ...
            <p v-else-if="loadError" class="mt-6 rounded-lg border border-danger/40 bg-danger/10 px-4 py-3 text-sm text-danger">
            ...
            <template v-else-if="manage">
                <form class="mt-6 grid gap-5" @submit.prevent="submit">
                    ...
                    <p v-if="form.errors.icon || form.errors.color" class="text-xs text-danger">{{ form.errors.icon ?? form.errors.color }}</p>
                    ...
                    >
                        <select v-model="form.preferred_domain_id" class="h-9">
                            ...
                            <option v-for="domain in manage.domains" :key="domain.id" :value="domain.id">{{ domain.hostname }}</option>
                        </select>
                    </Field>
                ...
                <div v-if="manage.role === 'owner'" class="mt-6 border-t pt-5">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div>
                                ...
                                <p class="mt-0.5 text-[13px] text-muted">
                                    {{ manage.can_delete ? 'Permanently removes this workspace and everything inside it.' : 'You must have another workspace before deleting this one.' }}
                                </p>
                            </div>
                    ...
                    <template v-else>
                        ...
                        <p class="mt-1 text-[13px] text-muted">
                            Links, domains, folders, members, and analytics in this workspace will be permanently deleted. This cannot be undone.
                        </p>
