...
async function generateInvite() {
    ...
    try {
        ...
        inviteError.value = error instanceof HttpError && error.status === 422 ? 'Invalid role selected.' : 'Could not generate an invite link. You can do it later from the Members page.';
    } finally {
        ...
        <div class="p-6">
            <template v-if="step === 'details'">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 text-sm text-muted">Use a workspace when links, members, domains, or folders should be isolated. Click the icon to change it.</p>
                    </div>
                </div>
            ...
            <template v-else>
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <h2 class="truncate text-base font-semibold text-foreground">Invite members to {{ createdWorkspace?.name }}</h2>
                        <p class="mt-0.5 text-sm text-muted">Anyone with the link joins this workspace with the link's role. You can skip this and invite later.</p>
                    </div>
                </div>
