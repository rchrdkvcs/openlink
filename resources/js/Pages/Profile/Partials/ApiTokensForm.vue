...
<template>
    <section>
        ...
        <div v-if="newToken" class="mt-6 rounded-lg border border-success/25 bg-success/10 p-4">
            ...
            <code class="mt-3 block break-all rounded-md border bg-background px-3 py-2 font-mono text-sm text-foreground">{{ newToken.token }}</code>
        </div>

        <form class="mt-6 flex max-w-md items-end gap-3" @submit.prevent="createToken">
            <div class="flex-1">
                <InputLabel for="token_name" value="Token name" />
                <TextInput id="token_name" v-model="form.name" class="mt-1.5 block w-full" placeholder="Browser extension" :disabled="!canCreate" />
                <InputError :message="form.errors.name" class="mt-2" />
                <p v-if="!canCreate" class="mt-2 text-xs text-warning">Verify your email before creating API tokens.</p>
            </div>
            <Button :loading="form.processing" :disabled="!canCreate">
                <KeyRound class="h-4 w-4" /> Create
            </Button>
        </form>
        ...
        <div class="mt-6 divide-y divide-border/60 rounded-lg border">
            <div v-for="token in tokens" :key="token.id" class="flex flex-wrap items-center gap-3 px-4 py-3">
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-faint">Created {{ formatDate(token.created_at) }} · Last used {{ formatDate(token.last_used_at) }}</p>
                </div>
